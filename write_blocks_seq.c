...
int main(int argc, char** argv) {
	...
	while(fgets(current_line, MAX_CHARS_PER_LINE, fp_read) != NULL) {
		...
		memcpy(&buffer[buffer_index], &record, sizeof(Record));

		if (buffer_index == records_per_block - 1) {
	...
	if (records_per_block - buffer_index > 0) {
		fwrite(buffer, sizeof(Record), buffer_index, fp_write);
		fflush(fp_write);
	}
